...
let
  ...
  headroomPropagatedBuildInputs = with python3.pkgs; [
    click
    rich
    pydantic
    tiktoken
...
python3.pkgs.buildPythonPackage {
  ...
  propagatedBuildInputs = headroomPropagatedBuildInputs;
  ...
  # headroom re-execs itself as a proxy subprocess via `sys.executable -m
  # headroom.cli proxy`. sys.executable is the bare interpreter from the
  # generated wrapper's shebang, which has no knowledge of the addsitedir
  # setup baked into that wrapper script — so the subprocess needs PYTHONPATH
  # exported explicitly to find the headroom module and its dependencies.
  postFixup = ''
    wrapProgram $out/bin/headroom \
      --set PYTHONPATH "$out/${python3.sitePackages}:${python3.pkgs.makePythonPath headroomPropagatedBuildInputs}"
  '';
